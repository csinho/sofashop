-- Estoque NULL na variação = usa estoque do produto; ambos NULL = sem controle de estoque.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    END IF;
    SELECT * INTO v_p FROM public.products p WHERE p.id = v_pid AND p.store_id = p_store_id;
    IF NOT FOUND THEN
    ...
    ELSE
      v_have := v_p.stock;
    END IF;
    IF v_have IS NOT NULL AND v_have < v_line_qty THEN
